...
commodity_pkl = pickle.load(open('encoders/encode_commodity.pkl', 'rb'))
scaler_pkl = pickle.load(open('models/scaler1.pkl', 'rb'))


model = pickle.load(open('models/modelv.pkl', 'rb'))
...
def predict():
    ...
    if request.method == "POST":
        ...
        print("Prediction: Rs.", prediction_unscaled_int)
        
    else:
        prediction = "Error"
        
    return render_template('services.html', prediction_text="The price of the crop will be: " + "Rs. " + str(round(prediction_unscaled_int)))
